...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.original_key = nn.Linear(config.hidden_size, self.all_head_size)
        self.value = nn.Linear(config.hidden_size, self.all_head_size)
        self.original_value = nn.Linear(config.hidden_size, self.all_head_size)
    ...
    def cross_key_attention(
        ...
        for i, qc_pair in enumerate(hidden_states):
            sep = sep_indices[i][0]
            # top indices to inform projection
        ...
        return torch.matmul(attention_probs, value_layer), attention_probs
    ...
    def forward(
        ...
        if MODE == -1:
            ...
            context_layer, attention_probs = self.cross_key_attention(hidden_states, sep_indices, attention_mask, MODE,
                                                                      max_ids)
