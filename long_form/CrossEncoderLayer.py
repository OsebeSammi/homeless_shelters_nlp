...
class CrossEncoderLayer(Module):
    ...
    def __setstate__(self, state):
        ...
        super(CrossEncoderLayer, self).__setstate__(state)
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        if self.norm_first:
            ...
            x = x + self._ca_block(self.norm3(x), src_mask, src_key_padding_mask)
            x = x + self._ff_block(self.norm2(x))
        else:
            x = self.norm1(x + self._sa_block(x, src_mask, src_key_padding_mask))
            x = self.norm3(x + self._ca_block(x, src_mask, src_key_padding_mask))
    ...
    # cross neighbour attention
    def _ca_block(self, x: Tensor,
                  attn_mask: Optional[Tensor], key_padding_mask: Optional[Tensor]) -> Tensor:
        x = self.self_attn(x, torch.roll(x, -1, 0), x,
                           attn_mask=attn_mask,
                           key_padding_mask=key_padding_mask,
                           need_weights=False)[0]
